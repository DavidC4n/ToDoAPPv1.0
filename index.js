...
// Obtener el elemento de la barra de búsqueda en el banner
const searchBar = document.querySelector('.search-bar input');

// Agregar un evento de escucha a la barra de búsqueda en el banner
searchBar.addEventListener('input', function(event) {
    const textoBusqueda = event.target.value.toLowerCase(); // Obtener el texto ingresado y convertirlo a minúsculas
    
    // Filtrar la lista de tareas
    const tareasFiltradas = LIST.filter(function(tarea) {
        return tarea.nombre.toLowerCase().includes(textoBusqueda); // Verificar si el nombre de la tarea incluye el texto de búsqueda
    });

    // Limpiar la lista de tareas en la interfaz
    lista.innerHTML = '';

    // Volver a cargar las tareas filtradas en la interfaz
